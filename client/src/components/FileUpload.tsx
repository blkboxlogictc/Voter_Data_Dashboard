...
export default function FileUpload({
  ...
  const handleProcessClick = async () => {
    if (!voterFile || !geoFile) {
      toast({
        title: "Missing files",
        description: "Please upload both voter data and district boundaries files",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <Card className="bg-white rounded-lg shadow mb-6">
      <CardContent className="p-6">
        <h2 className="text-xl font-medium mb-4 text-neutral-900">Data Upload</h2>
        
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-neutral-900 mb-2">Voter Data (JSON)</label>
            <div 
              className={`border-2 border-dashed rounded-lg p-5 text-center cursor-pointer transition ${
                voterDataDragActive ? "border-primary bg-primary/5" : "border-neutral-300 hover:border-primary hover:bg-primary/5"
              }`}
              onDragEnter={handleVoterDataDrag}
              onDragLeave={handleVoterDataDrag}
              onDragOver={handleVoterDataDrag}
              onDrop={handleVoterDataDrop}
              onClick={() => document.getElementById("voter-file-input")?.click()}
            >
              <input 
                id="voter-file-input" 
                type="file" 
              ...
              <div className="py-4">
                ...
                <p className="text-neutral-500 mb-1">Click or drag & drop your voter data file</p>
                <p className="text-sm text-neutral-400">Accepts JSON format</p>
              </div>
            </div>
            <div className="mt-2">
              {voterFile && (
                <div className={`flex items-center text-sm ${voterFile.isValid ? "text-green-600" : "text-red-600"}`}>
                  {voterFile.isValid ? (
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-900 mb-2">District Boundaries (GeoJSON)</label>
            <div 
              className={`border-2 border-dashed rounded-lg p-5 text-center cursor-pointer transition ${
                geoDataDragActive ? "border-primary bg-primary/5" : "border-neutral-300 hover:border-primary hover:bg-primary/5"
              }`}
            ...
            >
              ...
              <div className="py-4">
                ...
                <p className="text-neutral-500 mb-1">Click or drag & drop your district boundaries file</p>
                <p className="text-sm text-neutral-400">Accepts GeoJSON format</p>
              </div>
            </div>
            <div className="mt-2">
              {geoFile && (
                <div className={`flex items-center text-sm ${geoFile.isValid ? "text-green-600" : "text-red-600"}`}>
                  {geoFile.isValid ? (
                    <Check className="h-4 w-4 mr-1" />
                  ) : (
                    <AlertCircle className="h-4 w-4 mr-1" />
        ...
        <div className="mt-6 flex justify-center">
          <Button 
            ...
            disabled={!voterFile?.isValid || !geoFile?.isValid}
          >
            <UploadCloud className="h-5 w-5 mr-2" />
            <span>Process & Visualize Data</span>
          </Button>
        </div>
      </CardContent>
    </Card>
